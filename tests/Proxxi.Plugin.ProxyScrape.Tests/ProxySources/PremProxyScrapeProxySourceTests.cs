...
[TestFixture(TestOf = typeof(PremProxyScrapeProxySource))]
public class PremProxyScrapeProxySourceTests
{
    ...
    {
        IBatchProxySource proxySource = new PremProxyScrapeProxySource(_httpClient);

        await proxySource.InitializeAsync(new Dictionary<string, string> { { "apiKey", "test" } });
    ...
    {
        IStreamProxySource proxySource = new PremProxyScrapeProxySource(_httpClient);
